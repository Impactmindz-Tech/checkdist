...
import {
  getNotificationApi,
  notificationApi,
} from "@/utills/service/userSideService/userService/UserHomeService";
...
function Notification() {
  ...
  const getNotification = async () => {
    ...
    try {
      ...
      if (res?.isSuccess) {
        const { Message, Approvedtour, CancelledTour, NewTour, Support } =
          res.data;
  ...
  const handleToggleState = async (stateName, newState) => {
    ...
    const body = {
      ...
      Approvedtour:
        stateName === "Approved Tour" ? newState : approvedTourState,
      CancelledTour:
        stateName === "Cancelled Tours" ? newState : cancelledTourState,
      ...
      <div className="container">
        ...
        <div className="containts">
          ...
          <NotificationCard
            icon={Images.correct}
            title={"Approved Tour"}
            stateValue={approvedTourState}
            onToggle={(newState) =>
              handleToggleState("Approved Tour", newState)
            }
          />
          <NotificationCard
            icon={Images.closeCircle}
            title={"Cancelled Tours"}
            stateValue={cancelledTourState}
            onToggle={(newState) =>
