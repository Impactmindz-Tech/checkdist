...
import { paymentPaypalApi, paymentstripeApi, paypalcheckout } from "@/utills/service/userSideService/userService/UserHomeService";
...
function PaymentPage({ setPayemntDetails, payemntDetails, tourDetails }) {
  ...
  const handlecheckout = async () => {
    ...
    if (role.stripe) {
      ...
      try {
        ...
        let res = await paymentPaypalApi(body);
    ...
    <Drawer anchor="right" open={payemntDetails} onClose={() => setPayemntDetails(false)}>
      <div className="container px-10 py-10 sm:py-6 min-lg:w-[40vw] sm:w-[280px] sm:max-w-none sm:px-4">
        <header className="flex items-center mt-2">
          <div className="border cursor-pointer border-[#cccccc] w-[50px] h-[50px] sm:w-[36px] sm:h-[36px] p-2 rounded-full flex items-center justify-center" onClick={() => setPayemntDetails(false)}>
        ...
        <div className="mt-5 rounded-lg p-5 bg-[#add8e650] sm:p-3">
          <h1 className="text-lg sm:text-base">
            <span className="font-normal">Experience Name:</span> {tourDetails?.ExpId?.ExperienceName}
          </h1>
          <h1 className="text-lg sm:text-base">
            <span className="font-normal">Avatar Name:</span> {tourDetails?.ExpId?.avatarName}
          </h1>
          <h1 className="text-lg sm:text-base">
            <span className="font-normal">Amounts per Minute:</span> {tourDetails?.ExpId?.AmountsperMinute}
          </h1>
          <h1 className="text-lg sm:text-base">
            <span className="font-normal">end Time:</span> {formatTime(tourDetails?.endTime)}
          </h1>
          <h1 className="text-lg sm:text-base">
 ...
 <button  className="bg-black text-white font-semibold py-2 px-4 rounded hover:bg-black-600 transition duration-300">
    View Experience
  </button>
 </Link>
</div>
        ...
        <div className="flex flex-col gap-5 pt-5 sm:gap-2">
          <div className={`cursor-pointer ${role.paypal ? "border border-grey-900" : "border border-[#ccc]"} flex items-center gap-2  p-5 rounded-3xl relative sm:p-3 sm:rounded-lg text-sm`} onClick={() => changeSelectedState("paypal")}>
            ...
            {role.paypal && (
              <div className="tick bg-[#757575] rounded-full p-1 absolute right-5">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
              </div>
            )}
          </div>
          <div className={`cursor-pointer ${role.stripe ? "border border-grey-900" : "border border-[#ccc]"} flex items-center gap-2  p-5 rounded-3xl relative sm:p-3 sm:rounded-lg text-sm`} onClick={() => changeSelectedState("stripe")}>
            <img className="w-10 sm:w-6" src={Images.paypal} alt="paypal" />
          ...
          {/* <div className={`cursor-pointer ${role.applePay ? "border border-grey-900" : "border border-[#ccc]"} flex items-center gap-2  p-5 rounded-3xl relative sm:p-3 sm:rounded-lg text-sm`} onClick={() => changeSelectedState("applePay")}>
          ...
          {/* <div className={`cursor-pointer ${role.mastercard ? "border border-grey-900" : "border border-[#ccc]"} flex items-center gap-2 p-5 rounded-3xl relative  sm:p-3 sm:rounded-lg text-sm`} onClick={() => changeSelectedState("mastercard")}>
            <img className="w-10 sm:w-6" src={Images.mastercard} alt="mastercard" />
          ...
          <div onClick={handlecheckout}>
            <button className="cursor-pointer w-full border border-primaryColor-900 p-4 text-center text-grey-900 mt-8 rounded-xl font-bold sm:p-3 sm:rounded-lg sm:text-sm">Pay</button>
          </div>
        </div>
      </div>
    </Drawer>
