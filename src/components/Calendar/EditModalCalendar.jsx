...
const EditModalCalendar = ({ date, onDateChange }) => {
  ...
  let eid = getLocalStorage("currentBooking");
  ...
  useEffect(() => {
    const fetchBookingSlots = async () => {
      ...
      try {
        ...
        const response = await bookingSlotsApi(eid , body); // Call the API
  ...
  const handleTimeSlotClick = (slot) => {
    ...
    // Set time in 24-hour format for backend
  };
      ...
      <button disabled
        ...
        className={`p-2 rounded-md w-full mt-2 border border-black text-black font-semibold ${
          selectedSlot === slot.avatarTime // Check if this time is selected
            ...
            : "bg-white cursor-not-allowed " // Unselected slot styling
        }`}
